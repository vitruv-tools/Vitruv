...
/**
 * Manages the trigger file used for inter-process communication between the Git {@code post-commit} hook and
 ...
 *   <li>The file contains the real commit SHA assigned by Git, the branch name, a request identifier, and a timestamp.</li>
 *   <li>The watcher polls for the file, generates the semantic changelog using the real commit SHA, auto-stages it, and deletes the trigger file.</li>
 * </ul>
 *
 * <p>Unlike the pre-commit trigger, the post-commit trigger is fire-and-forget: the hook does not wait for a result file.
 * The changelog is generated asynchronously in the background while the developer continues working.
 *
 * <p>The commit SHA in this trigger is the real SHA assigned by Git,  not the parent SHA read before the commit was created.
 * This is what makes the changelog traceable via {@code git log}: the changelog file name and content both match the actual commit.
 *
...
public class PostCommitTriggerFile extends AbstractTriggerFile<PostCommitTriggerFile.TriggerInfo> {
     ...
     * The trigger file will be located at {@code <repositoryRoot>/.vitruvius/post-commit-trigger}.
     *
    ...
    /**
     * Creates the trigger file to signal that a semantic changelog should be generated for the given commit.
     * A unique request identifier is generated for traceability.
     * <p>File format: {@code commitSha|branch|requestId|timestamp}
     * <p>The {@code commitSha} must be the real SHA of the commit just created, not the parent SHA.
     * On the command line, the post-commit hook reads this via {@code git rev-parse HEAD}, which at post-commit time points to the new commit.
     *
    ...
    public String createTrigger(String commitSha, String branch) throws IOException {
        ...
        LOGGER.debug("Created post-commit trigger: commit={}, branch={}, requestId={}", commitSha.substring(0, Math.min(7, commitSha.length())), branch, requestId);
    ...
    protected TriggerInfo parseTriggerInfo(String[] parts) {
        ...
        LOGGER.debug("Post-commit trigger parsed: commit={}, branch={}, requestId={}", commitSha.substring(0, Math.min(7, commitSha.length())), branch, requestId);
     ...
     *
     * <p>The {@code commitSha} is the real SHA assigned by Git to the new commit, not the parent SHA.
     * This ensures the generated changelog file name and content match the actual commit in {@code git log}.
     */
    @Getter
