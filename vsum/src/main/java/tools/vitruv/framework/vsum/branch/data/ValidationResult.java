 ...
 *
 * <p>A result is either valid (the commit may proceed) or invalid (the commit should be
 * blocked). In both cases, warning messages may be present to inform the developer of
 * non-blocking issues detected during validation.
 *
 * <p>Instances are created exclusively through the static factory methods
 * ({@link #success()}, {@link #successWithWarnings}, {@link #failure},
 * {@link #failureWithWarnings}) rather than through a public constructor.
 * This makes the intent of each result explicit at the call site and prevents invalid
 * combinations such as a valid result that carries errors.
 *
 * <p>All instances are immutable and thread-safe. The error and warning lists are copied
 * on construction and exposed as unmodifiable views.
 */
@Getter
public class ValidationResult {
   ...
   * The list is unmodifiable; any attempt to mutate it will throw
  ...
  /**
   * Private constructor to enforce use of the static factory methods, which guarantee
   * that the valid flag and the presence of errors are always consistent.
   */
  private ValidationResult(boolean valid, List<String> errors, List<String> warnings) {
    // List.copyOf() both copies the content and returns an unmodifiable view, ensuring
    // that callers cannot mutate the lists through the original references they passed in.
    this.valid = valid;
  ...
  /**
   * Creates a successful validation result with non-blocking warnings. The VirtualModel
   * is consistent enough to allow the commit, but the developer should be informed of
   * the issues described in the warning messages.
   *
  ...
  /**
   * Creates a failed validation result. The commit should be blocked until the issues
   * described in the error messages are resolved.
   *
  ...
  /**
   * Creates a failed validation result with both errors and warnings. The commit should
   * be blocked, and the warnings provide additional context about related issues.
   *
   * @param errors   error messages describing why validation failed, must not be null.
  ...
  /**
   * Returns true if any warning messages are present. A result can have warnings
  ...
  /**
   * Returns true if any error messages are present. An invalid result always has at least
   * one error. A valid result never has errors.
   */
