...
public class ReloadTriggerFile extends AbstractTriggerFile<ReloadTriggerFile.TriggerInfo> {
   ...
   * <p>File format: {@code branchName|requestId|timestamp}
   *
   * @param branchName    the name of the branch that was just checked out. Must not be null.
   * @param oldBranchName the name of the branch that was active before checkout.
   *                      Must not be null.
  ...
  protected TriggerInfo parseTriggerInfo(String[] parts) {
    if (parts.length != 4 && parts.length != 3 && parts.length != 1) {
      LOGGER.warn("Invalid reload trigger field count: expected 4, 3, or 1, got {}",
          parts.length);
      return null;
    }
    ...
    if (parts.length == 4) {
      ...
      timestamp = System.currentTimeMillis();
      oldBranchName = "unknown";
      LOGGER.warn("Legacy reload trigger format - oldBranch unknown, requestId='{}'", requestId);
    }
  ...
  public static class TriggerInfo extends AbstractTriggerFile.TriggerInfo {
    ...
    /**
     * The name of the branch that was just checked out.
     */
    private final String branchName;

    /**
     * The name of the branch that was active before the checkout.
     */
     ...
     *
     * @param branchName    the branch that was just checked out. Must not be null.
     * @param oldBranchName the branch that was active before the checkout. Must not be null.
