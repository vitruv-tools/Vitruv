...
public class VsumFileSystemLayout {
    ...
    private Path getMetadataFilePath(String... metadataKey) {
        checkArgument(metadataKey != null && metadataKey.length > 0,
                "The key must have at least one part!");
        checkArgument(metadataKey[metadataKey.length - 1].contains("."),
        ...
        for (String keyPart : metadataKey) {
            ...
            assert keyPart != null;
            preparedKeyPart = switch (keyPart) {
                case "." -> "_.";
                case ".." -> "_._.";
                case "" -> "_";
     ...
     * 
     * @param metadataKey The key uniquely identifying the metadata model. The different parts of
     *        the key can be used to convey some sort of hierarchy in the metadata. The key may
     *        contain arbitrary characters. The last key part contains the metadata model's file
     *        name and extension.
    ...
    public URI getConsistencyMetadataModelURI(String... metadataKey) {
        checkPrepared();
        Path metadataPath =
                getConsistencyMetadataFolder().resolve(getMetadataFilePath(metadataKey));
