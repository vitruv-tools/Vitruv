...
/**
 * Validates the VSUM for consistency before allowing a Git commit to proceed, and
 * generates the semantic changelog entry for the commit.
 *
...
public class PreCommitHandler {
  ...
  /**
   * Validates the VirtualModel for consistency. Returns a {@link ValidationResult} that
   * indicates whether the commit is safe to proceed, with error messages describing any
   * detected inconsistencies and warning messages for non-blocking issues.
   *
   * <p>If an unexpected exception is thrown during any validation step, it is caught,
   * logged, and added to the error list rather than propagated, so that the caller always
   * receives a well-formed result.
   *
  ...
  public ValidationResult validate() {
    ...
    LOGGER.info("VirtualModel validation completed: valid={}, errors={}, warnings={}",
   ...
   * Resources that are not yet loaded are loaded eagerly so that parse errors are
  ...
  /**
   * Checks that no object in any loaded resource is an unresolved proxy, and that no
   * non-containment cross-reference points to a proxy.
   * Unresolved proxies indicate that the VirtualModel references objects in files that
          ...
          for (EReference ref : obj.eClass().getEAllReferences()) {
            if (!ref.isContainment()) {
              ...
              if (value instanceof EObject && ((EObject) value).eIsProxy()) {
                errors.add("Unresolved reference '" + ref.getName() + "' in "
                    + EcoreUtil.getURI(obj) + " points to proxy "
                    + EcoreUtil.getURI((EObject) value));
                proxyCount++;
              } else if (value instanceof Collection) {
                for (Object item : (Collection<?>) value) {
                  if (item instanceof EObject && ((EObject) item).eIsProxy()) {
                    errors.add("Unresolved reference in collection '"
                        + ref.getName() + "' in "
                        + EcoreUtil.getURI(obj) + " points to proxy");
                    proxyCount++;
                  }
  ...
  /**
   * Checks that the correspondence model is accessible. Detailed validation of individual
   * correspondence links is planned for a future iteration once the correspondence model
