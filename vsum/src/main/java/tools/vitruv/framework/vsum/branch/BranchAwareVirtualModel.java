 ...
 *
 * <p>This class implements the decorator pattern: it wraps a single {@link InternalVirtualModel}
 * instance and delegates all normal V-SUM operations to it. Branch switching is handled by
 * {@link #switchBranch(String, String)}, which reloads the wrapped model in place using the
 * target branch's {@link VsumFileSystemLayout} without disposing or recreating the object.
 *
 ...
 * registry conflicts. One instance reloaded per branch switch is sufficient and memory-efficient.
 */
public class BranchAwareVirtualModel implements InternalVirtualModel {
    ...
    /**
     * Root of the Git repository. Used to construct per-branch {@link VsumFileSystemLayout} instances in {@link #switchBranch(String, String)}.
     */
    private final Path repoRoot;

    /**
     * Name of the currently active branch. Kept in sync with the layout inside {@link #activeModel} after every successful switch.
    ...
    /**
     * Switches the active V-SUM state to the given target branch by reloading the wrapped {@link InternalVirtualModel}
     * in place with the target branch's {@link VsumFileSystemLayout}.
     * <p>This method is called by {@link tools.vitruv.framework.vsum.branch.handler.VsumReloadWatcher} when a Git {@code post-checkout} hook trigger is detected.
     *
     * <p>Steps performed:
     * <ol>
     *   <li>Constructs a {@link VsumFileSystemLayout} for the target branch using {@link VsumFileSystemLayout#forBranch(Path, String)} </li>
     *   <li>Calls {@link VsumFileSystemLayout#prepare()} to create the target branch's VSUM directory on disk if it does not exist yet.</li>
     *   <li>Calls {@link VsumFileSystemLayout#inheritFromBranchIfEmpty(String)} so a freshly
     *       created branch starts with the parent branch's UUID and correspondence state.</li>
    ...
    public void switchBranch(String oldBranch, String newBranch) throws BranchOperationException {
        ...
        try {
            // Use forBranch() to bypass JGit HEAD resolution as we already know the target branch name from the trigger file,
            ...
            // Reload in place: VirtualModelImpl unloads all the current branch's resources, resets the UUID resolver,
            // recreates the correspondence model pointing to the target branch's file, and reloads from disk.
            ...
            throw new BranchOperationException("Failed to switch V-SUM to branch '" + newBranch + "'", e);
        }
    }
    ...
    /**
     * Disposes the wrapped V-SUM, releasing all resources and deregistering from {@code VirtualModelRegistry}.
