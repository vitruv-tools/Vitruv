...
/**
 * Manages the trigger file used for inter-process communication between the Git {@code post-merge} hook and {@link tools.vitruv.framework.vsum.branch.handler.VsumMergeWatcher}.
 *
 * <ul>
 *   <li>The Git post-merge hook creates this file after a merge completes.</li>
 *   <li>The file contains the merge commit SHA, source branch, target branch, request identifier, and timestamp.</li>
 *   <li>The watcher polls for the file, validates the merged VSUM state, writes the result and merge metadata, and deletes the trigger file.</li>
 *   <li>Unlike the pre-commit hook, post-merge validation is non-blocking - the merge has already completed. The result is written as a warning report if the merged state contains inconsistencies.</li>
 * </ul>
 *
 * <p>The trigger carries both branches involved in the merge because the merge metadata written to {@code .vitruvius/merges/<sha>.metadata} must record the full merge context: which branch was merged in (source) and which branch received the merge (target).
 *
 * <p>File format:
...
public class MergeTriggerFile extends AbstractTriggerFile<MergeTriggerFile.TriggerInfo> {
     ...
     * A unique request identifier is generated and returned so that the hook can later locate the correct result file.
     *
     * <p>File format:
     * {@code mergeCommitSha|sourceBranch|targetBranch|requestId|timestamp}
     *
     * @param mergeCommitSha the SHA of the merge commit created by Git, must not be null.
     * @param sourceBranch   the branch that was merged in (e.g. {@code feature-x}), must not be null.
     * @param targetBranch   the branch that received the merge (e.g. {@code main}), must not be null.
     ...
     */
    public String createTrigger(String mergeCommitSha, String sourceBranch, String targetBranch) throws IOException {
        ...
        writeTrigger(String.format("%s%s%s%s%s%s%s%s%d", mergeCommitSha, DELIMITER, sourceBranch, DELIMITER, targetBranch, DELIMITER, requestId, DELIMITER, timestamp));

        LOGGER.debug("Created merge trigger: mergeCommit={}, source={}, target={}, requestId={}", mergeCommitSha.substring(0, Math.min(7, mergeCommitSha.length())), sourceBranch, targetBranch, requestId);
     ...
     * No legacy format is supported for merge triggers because the two-branch context is essential for writing correct merge metadata.
     *
    ...
    protected TriggerInfo parseTriggerInfo(String[] parts) {
        ...
        if (mergeCommitSha.isEmpty() || sourceBranch.isEmpty() || targetBranch.isEmpty() || requestId.isEmpty()) {
            LOGGER.warn("Empty required field in merge trigger, discarding");
            return null;
        }

        LOGGER.debug("Merge trigger parsed: mergeCommit={}, source={}, target={}, requestId={}", mergeCommitSha.substring(0, Math.min(7, mergeCommitSha.length())), sourceBranch, targetBranch, requestId);
     ...
     * <p>Identifies a single post-merge validation request: the merge commit SHA, the source branch that was merged in,
     * the target branch that received the merge, a unique request identifier for matching the result file, and a timestamp for debugging and timeout detection.
     */
    @Getter
    public static class TriggerInfo extends AbstractTriggerFile.TriggerInfo {
         ...
         */
        public TriggerInfo(String mergeCommitSha, String sourceBranch, String targetBranch, String requestId, long timestamp) {
            super(requestId, timestamp);
            this.mergeCommitSha = Objects.requireNonNull(mergeCommitSha, "merge commit SHA must not be null");
        ...
        @Override
        public int hashCode() {
            return Objects.hash(mergeCommitSha, sourceBranch, targetBranch, getRequestId(), getTimestamp());
        }
