 ...
 *   <li>The watcher polls for the file, validates the merged VSUM state, writes
 *       the result and merge metadata, and deletes the trigger file.</li>
 *   <li>Unlike the pre-commit hook, post-merge validation is non-blocking — the
 *       merge has already completed. The result is written as a warning report
 *       if the merged state contains inconsistencies.</li>
 * </ul>
 *
 * <p>The trigger carries both branches involved in the merge because the merge
 * metadata written to {@code .vitruvius/merges/<sha>.metadata} must record the
 * full merge context (MG-6): which branch was merged in (source) and which branch
 * received the merge (target).
 *
 * <p>File format:
...
public class MergeTriggerFile extends AbstractTriggerFile<MergeTriggerFile.TriggerInfo> {
     ...
     * A unique request identifier is generated and returned so that the hook can
     * later locate the correct result file.
     *
     * <p>File format:
     * {@code mergeCommitSha|sourceBranch|targetBranch|requestId|timestamp}
     *
     * @param mergeCommitSha the SHA of the merge commit created by Git. Must not be null.
     * @param sourceBranch   the branch that was merged in (e.g. {@code feature-x}).
     *                       Must not be null.
     * @param targetBranch   the branch that received the merge (e.g. {@code main}).
     *                       Must not be null.
     ...
     */
    public String createTrigger(String mergeCommitSha, String sourceBranch, String targetBranch)
            throws IOException {
        ...
        writeTrigger(String.format("%s%s%s%s%s%s%s%s%d",
                mergeCommitSha, DELIMITER,
                sourceBranch,   DELIMITER,
                targetBranch,   DELIMITER,
                requestId,      DELIMITER,
                timestamp));
     ...
     *
     * <p>Returns {@code null} if the field count is not five or any required field
     * is empty. No legacy format is supported for merge triggers because the
     * two-branch context is essential for writing correct merge metadata.
     *
     ...
     *
     * <p>Identifies a single post-merge validation request: the merge commit SHA,
     * the source branch that was merged in, the target branch that received the
     * merge, a unique request identifier for matching the result file, and a
    ...
    public static class TriggerInfo extends AbstractTriggerFile.TriggerInfo {
         ...
         */
        public TriggerInfo(String mergeCommitSha, String sourceBranch, String targetBranch,
                           String requestId, long timestamp) {
            super(requestId, timestamp);
            this.mergeCommitSha = Objects.requireNonNull(mergeCommitSha,
                    "merge commit SHA must not be null");
            this.sourceBranch   = Objects.requireNonNull(sourceBranch,
                    "source branch must not be null");
            this.targetBranch   = Objects.requireNonNull(targetBranch,
                    "target branch must not be null");
        }
        ...
        @Override
        public int hashCode() {
            return Objects.hash(mergeCommitSha, sourceBranch, targetBranch,
