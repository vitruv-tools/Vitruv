...
/**
 * Background daemon that polls for a reload trigger file and reloads the VSUM when a branch
 * switch is detected.
 *
 * <p>Logic:
 * <ul>
 ...
 * </ul>
 *
 * <p>This file-based approach ensures the VirtualModel stays consistent even when developers
 * perform branch switches directly through the Git command line rather than through the
 * Vitruvius API.
 *
 * <p>Concurrency protection: a {@link FileLock} on {@code .vitruvius/.reload.lock} ensures
 * that only one reload runs at a time. If the lock cannot be acquired because another reload
 * is already in progress, the trigger file is re-created so the request is retried on the
 * next poll cycle.
 */
public class VsumReloadWatcher {
   ...
   * @param repositoryRoot the root directory of the Git repository. The trigger file and
   *                       lock file will be located under {@code .vitruvius/} inside this
   *                       directory.
   */
  ...
  /**
   * Stops the watcher thread and waits up to {@value #STOP_TIMEOUT_MS} milliseconds for it
  ...
  /**
   * Main loop executed by the background thread. Polls for the trigger file and delegates
   * to {@link #handleReloadRequest} when a trigger is found.
  ...
  /**
   * Handles a single reload request with OS-level lock protection to prevent concurrent
   * reloads from corrupting the VirtualModel state.
   *
   * <p>If the lock cannot be acquired because another reload is already in progress, the
   * trigger is re-created so that the request is retried on the next poll cycle.
   * This approach is consistent with the validation watcher and avoids silently dropping
   * branch switch events under load.
   *
  ...
  private void handleReloadRequest(ReloadTriggerFile.TriggerInfo info) {
    ...
    try {
      ...
      LOGGER.error("Unexpected error handling reload request (requestId='{}')",
          info.getRequestId(), e);
    }
  }
   ...
   * The old branch name is passed as {@code "unknown"} because the watcher receives only
   * the new branch name from the trigger file. The previous branch is not available in the
   * fire-and-forget inter-process communication path.
   *
  ...
  private void performSwitch(ReloadTriggerFile.TriggerInfo info) {
    try {
      ...
      LOGGER.info("VirtualModel reloaded in {}ms (branch='{}' <- '{}', requestId='{}')",
          duration, info.getBranchName(), info.getOldBranchName(), info.getRequestId());
    } catch (BranchOperationException e) {
      LOGGER.error("Failed to reload after branch switch (branch='{}', requestId='{}')",
          info.getBranchName(), info.getRequestId(), e);
    }
  }
}
