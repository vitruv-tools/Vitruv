...
public final class VirtualModelTestUtil {
    ...
    /**
     * Create a recorder, start recording a resource set, apply changes, stop and return the
     * recorded changes.
     */
    public static VitruviusChange<Uuid> recordChanges(ResourceSet resourceSet,
            UuidResolver uuidResolver, Runnable changesToPerform) {
        ...
        var resolvedChange = changeResolver.assignIds(result);
        recorder.close();
    ...
    public static VirtualModel createAndLoadTestVirtualModel(Path folder) throws Exception {
        return new VirtualModelBuilder().withStorageFolder(folder)
                .withUserInteractor(UserInteractionFactory.instance
                        .createUserInteractor(UserInteractionFactory.instance
                                .createPredefinedInteractionResultProvider(null)))
                .buildAndInitialize();
    }
    ...
    /**
     * Creates a model resource URI for a given project folder. Use the suffix to distinguish
     * multiple URIs.
     */
    ...
    public static class RedundancyChangePropagationSpecification
        ...
        public static URI getTargetResourceUri(URI sourceUri) {
            ...
            if (sourceUriWithoutFileExtension.endsWith(copySuffix)) {
                String sourceUriWithoutSuffix = sourceUriWithoutFileExtension.substring(0,
                        sourceUriWithoutFileExtension.length() - copySuffix.length());
                return URI.createFileURI(sourceUriWithoutSuffix + "." + sourceUri.fileExtension());
            } else {
                return URI.createFileURI(sourceUriWithoutFileExtension + copySuffix + "."
                        + sourceUri.fileExtension());
            }
        }
        ...
        public void propagateChange(EChange<EObject> change,
            ...
            List<Root> correspondingRoots =
                    correspondenceModel.getCorrespondingEObjects(insertedRoot).stream()
                            .filter(obj -> obj instanceof Root).map(obj -> (Root) obj)
            ...
            if (insertedRoot.eContainer() != null) {
                List<Root> correspondingObjects = correspondenceModel
                        .getCorrespondingEObjects(insertedRoot.eContainer(), null).stream()
                        .filter(obj -> obj instanceof Root).map(obj -> (Root) obj)
                        .collect(Collectors.toList());
