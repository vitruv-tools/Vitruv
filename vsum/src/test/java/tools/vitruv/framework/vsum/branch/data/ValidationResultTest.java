 ...
 * <p>The tests cover each factory method, the predicate methods
 * {@link ValidationResult#hasErrors()} and {@link ValidationResult#hasWarnings()},
 * the immutability guarantee stated in the class Javadoc,
 * and the {@code toString} representation used in log output.
 */
class ValidationResultTest {
    ...
    /**
     * Verifies that {@link ValidationResult#successWithWarnings(List)} produces a valid result
     * that carries the provided warnings.
     * The VirtualModel is consistent enough to allow the commit,
     * but the developer must be informed of the non-blocking issues.
     */
    @Test
    ...
    void successWithWarningsCreatesValidResult() {
        ...
        assertEquals("Warning 1", result.getWarnings().get(0),
                "warning messages must be preserved in the original order");
    }

    /**
     * Verifies that {@link ValidationResult#failure(List)} produces an invalid result
     * that carries the provided error messages.
    ...
    void failureCreatesInvalidResult() {
        ...
        assertFalse(result.hasWarnings(),
                "a failure result without explicit warnings must not have warnings");
        assertEquals(2, result.getErrors().size());
        assertEquals("Error 1", result.getErrors().get(0),
                "error messages must be preserved in the original order");
    }

    /**
     * Verifies that {@link ValidationResult#failureWithWarnings(List, List)} produces an invalid result
     * that carries both errors and warnings.
    ...
    void failureWithWarningsCreatesInvalidResult() {
        var result = ValidationResult.failureWithWarnings(
                List.of("Error 1"), List.of("Warning 1", "Warning 2"));
     ...
     * The class Javadoc states that all instances are immutable,
     * so external code must not be able to mutate the internal state of a result after it is created.
     */
    @Test
    ...
    void listsAreUnmodifiable() {
        ...
        assertThrows(UnsupportedOperationException.class, ()
                -> result.getErrors().add("extra error"),
                "the errors list must not allow external modification");
        assertThrows(UnsupportedOperationException.class, ()
                -> result.getWarnings().add("extra warning"),
                "the warnings list must not allow external modification");
    }

    /**
     * Verifies that passing a null list to any factory method throws a {@link NullPointerException}.
     * The internal constructor uses {@code List.copyOf()} which rejects null,
     * so callers receive a clear failure immediately rather than
     * a silent empty list or a delayed null pointer elsewhere.
     */
    @Test
    ...
    void factoryMethodsRejectNullLists() {
        assertThrows(NullPointerException.class, () -> ValidationResult.failure(null),
                "null error list must be rejected by failure()");
        assertThrows(NullPointerException.class, () -> ValidationResult.successWithWarnings(null),
                "null warning list must be rejected by successWithWarnings()");
        assertThrows(NullPointerException.class, () -> ValidationResult.failureWithWarnings(null, List.of()),
                "null error list must be rejected by failureWithWarnings()");
        assertThrows(NullPointerException.class, () -> ValidationResult.failureWithWarnings(List.of(), null),
                "null warning list must be rejected by failureWithWarnings()");
    }

    /**
     * Verifies that {@link ValidationResult#toString()} produces a string that includes the validity flag
     * and, when present, the counts of errors and warnings.
    ...
    void toStringIncludesValidityAndCounts() {
        ...
        assertTrue(success.toString().contains("valid=true"),
                "toString must include valid=true for a success result");
        ...
        assertTrue(failure.toString().contains("errors=2"),
                "toString must include the error count for a failure result");
    }
}