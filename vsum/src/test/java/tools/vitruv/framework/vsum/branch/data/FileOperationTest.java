 ...
 *
 * <p>Because {@link FileOperation} is a pure enum with no behavior, the tests focus on the contract that other classes depend on:
 * the exact set of declared constants and their string representation.
 * Any accidental addition, removal, or renaming of a constant would break {@link FileChange} validation logic and changelog serialization,
 * so these properties are worth protecting with explicit assertions.
 */
class FileOperationTest {

    /**
     * Verifies that exactly the five expected operation constants are declared and that each constant can be round-tripped through {@link FileOperation#valueOf(String)}.
     * This protects against accidentally adding or removing a constant, and confirms that the string names used in serialized changelogs
    ...
    void allExpectedConstantsDeclaredAndRoundTrip() {
        // the exact count matters because other classes (such as FileChange validation and changelog serialization) depend on the specific set of operation types.
        assertEquals(5, FileOperation.values().length, "exactly five operation constants must be declared");
     ...
     * The string representation is used when writing operation names to changelog files, so the exact format must be stable and predictable.
     */
    @Test
    ...
    /**
     * Verifies that {@link FileOperation#valueOf(String)} throws an {@link IllegalArgumentException} for an unrecognized name.
     * This ensures that changelog files containing an unknown operation type are rejected at parse time rather than silently producing a null or default value.
     */
    @Test
    @DisplayName("valueOf throws an exception for an unrecognized operation name")
    void valueOfThrowsForUnknownName() {
        // an unrecognized name such as one that might appear in a manually edited changelog file must be rejected immediately with a clear exception.
