...
/**
 * A specific view type that is able to create and update views. This is not its public interface
 * but only for internal usage by views and their selectors.
 *
...
public interface ViewCreatingViewType<S extends ViewSelector, Id> extends ViewType<S> {
  /**
   * Creates a view for the given {@link ViewSelector}. The selector must have been created by
   * calling the {@link #createSelector} method of the same {@link ViewCreatingViewType}.
   *
  ...
  /**
   * Updates a view that is created from this view type to ensure it is consistent with the virtual
   * model.
   *
