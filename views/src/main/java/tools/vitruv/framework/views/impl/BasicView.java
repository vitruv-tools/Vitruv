...
class BasicView implements ModifiableView, ChangePropagationListener {
    ...
    protected BasicView(ViewCreatingViewType<? extends ViewSelector, HierarchicalId> viewType,
            ChangeableViewSource viewSource, ViewSelection selection) {
    ...
    public void close() throws Exception {
        if (!closed) {
            closed = true;
            viewResourceSet.getResources().forEach(resource -> {
                try {
                    resource.unload();
                } catch (Exception e) {
                    e.printStackTrace();
