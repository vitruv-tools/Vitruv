 ...
 * by the domains when there are no change sequences available and changes need
 * to be propagated
...
public interface StateBasedChangeResolutionStrategy {
   ...
   * change sequences.
   * Changes must use hierarchical IDs and are returned unresolved. The
   * {@code oldState} remains
   * unmodified from calling this method.
   *
   * @param newState is the new state of the resource, must not be
   *                 <code>null</code> and must not
   *                 contain proxies.
