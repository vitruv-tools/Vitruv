...
class JMLInvariantExpressionTransformations extends JML2JavaTransformationsBase {
	...
	override replaceNonRootEObjectSingle(EObject affectedEObject, EReference affectedReference, EObject oldValue,
		...
		if (affectedReference == JMLPackage.eINSTANCE.JMLExpressionHaving_Expr) {
			...
			val affectedModelInstanceObject = (affectedEObject as JMLInvariantExpression).getModelInstanceElement
			LOGGER.trace("The expression " + ConcreteSyntaxHelper.convertToConcreteSyntax(oldValue) +
			...
			val addedReferences = elementReferencesChanged.filter[changed | !elementReferencesOriginal.exists[EcoreUtil.equals(changed, it)]]
			val addedReferencesWithoutHelper = addedReferences.filter[target instanceof Member].filter[!(target as Member).hasJMLHelperModifier(shadowCopy2)]
			
			val removedReferences = elementReferencesOriginal.filter[orig | !elementReferencesChanged.exists[EcoreUtil.equals(orig, it)]]
			val removedReferencesWithRemovableHelper = removedReferences.filter[target instanceof Member].filter[!(target as Member).referencesFromInvariantsExist(shadowCopy, it)]
			...
			for (removedReference : removedReferencesWithRemovableHelper) {
				...
				if (jmlElement != null) {
					val jmlModelInstanceElement = jmlElement.modelInstanceElement
					val jmlMemberDecl = Utilities.getMemberDecl(jmlModelInstanceElement)
					jmlMemberDecl.jmlModifiers.removeAll(jmlMemberDecl.jmlModifiers.filter[modifier == JMLSpecMemberModifier.HELPER])
					changedObjects += jmlModelInstanceElement
				}
			}
	...
	private def referencesFromInvariantsExist(Member member, ShadowCopy shadowCopy, ElementReference referenceToIgnore) {
		...
		for (setting : results) {
			if (setting.EObject != referenceToIgnore && setting.EObject instanceof ElementReference && setting.EStructuralFeature == ReferencesPackage.eINSTANCE.elementReference_Target) {
