...
class JavaMethodTransformations extends Java2JMLTransformationBase {
	...
	override setCorrespondenceForFeatures() {
		featureCorrespondenceMap.put(MembersPackage.eINSTANCE.method.getEStructuralFeature("annotationsAndModifiers"), JMLPackage.eINSTANCE.modifiable_Modifiers)
		featureCorrespondenceMap.put(MembersPackage.eINSTANCE.method.getEStructuralFeature("parameters"), JMLPackage.eINSTANCE.methodDeclaration_Parameters)
		featureCorrespondenceMap.put(MembersPackage.eINSTANCE.method.getEStructuralFeature("name"), JMLPackage.eINSTANCE.identifierHaving_Identifier)
		featureCorrespondenceMap.put(MembersPackage.eINSTANCE.method.getEStructuralFeature("typeReference"), JMLPackage.eINSTANCE.typed_Type)
		featureCorrespondenceMap.put(MembersPackage.eINSTANCE.method.getEStructuralFeature("exceptions"), JMLPackage.eINSTANCE.methodDeclaration_Exceptions)
	...
	override replaceNonRootEObjectSingle(EObject newAffectedEObject, EObject oldAffectedEObject, EReference affectedReference,	EObject oldValue, EObject newValue) {
		...
		if (jmlFeature == JMLPackage.eINSTANCE.typed_Type) {
			changedObjects.addAll(CommonSynchronizerTransformations.replaceNonRootEObjectSingleType(javaMethod, oldValue as TypeReference, newValue as TypeReference, correspondenceInstance))
		}
	...
	override updateSingleValuedEAttribute(EObject affectedEObject, EAttribute affectedAttribute, Object oldValue, Object newValue) {
		...
		if (jmlFeature == JMLPackage.eINSTANCE.identifierHaving_Identifier) {
			...
			val jmlCoid = dummy.containingConcreteClassifier.getSingleCorrespondingEObjectOfType(ClassOrInterfaceDeclaration)
			...
			val jmlMethodDeclarations = correspondenceInstance.getAllCorrespondingEObjects(affectedEObject).filter(MethodDeclaration)
	...
	override createNonRootEObjectInList(EObject newAffectedEObject, EObject oldAffectedEObject, EReference affectedReference, EObject newValue, int index, EObject[] newCorrespondingEObjects) {
		...
		if (jmlFeature == JMLPackage.eINSTANCE.methodDeclaration_Parameters) {
			...
			return CommonSynchronizerTransformations.createNonRootEObjectInList(oldAffectedEObject, newValue as Modifier, correspondenceInstance)
		...
		if (jmlFeature == JMLPackage.eINSTANCE.methodDeclaration_Parameters) {
			...
			if (jmlMethodDeclaration != null) {
				...
				if (!simulateElementRemoval(oldValue as Parameter)) {
					userInteracting.showMessage(UserInteractionType.MODAL, "The change is not allowed since this parameter is referenced in this method or in specifications.");
					return createTransformationChangeResultAborted
				}
				...
				correspondenceInstance.update(jmlMethodDeclarationOldTUID, jmlMethodDeclaration.TUID)
				
				changedObjects.add(jmlMethodDeclaration)
			}
		} else if (jmlFeature == JMLPackage.eINSTANCE.modifiable_Modifiers) {
			val jmlMemberDeclWithModifier = getSingleCorrespondingEObjectOfType(oldAffectedEObject, MemberDeclWithModifier)
	...
	override replaceNonRootEObjectInList(EObject affectedEObject, EReference affectedReference, EObject oldValue, EObject newValue, int index, EObject[] oldCorrespondingEObjectsToDelete, EObject[] newCorrespondingEObjects) {
		...
		LOGGER.trace("ReplaceNonRootEObjectInList\t" + affectedEObject + "." + affectedReference.name + " (" + oldValue + " @ " + index + " -> " + newValue + ")")
		
		val jmlFeature = featureCorrespondenceMap.claimValueForKey(affectedReference)
		
		if (jmlFeature == JMLPackage.eINSTANCE.modifiable_Modifiers) {
			return CommonSynchronizerTransformations.replaceNonRootEObjectInList(affectedEObject, oldValue as Modifier, newValue as Modifier, correspondenceInstance)
		}
