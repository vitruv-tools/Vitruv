...
abstract class Java2JMLTransformationBase extends EObjectMappingTransformationBase {
	...
	private def dispatch renameAllParentsIfApplicable(Method m, String newName) {
		...
		for (superClassifier : classifier.allSuperClassifiers) {
			val candidates = superClassifier.methods.filter[name.equals(m.name)]
				.filter[parameters.size == m.parameters.size]
				.filter[StringOperationsJaMoPP.getStringRepresentation(typeReference, arrayDimension).equals(StringOperationsJaMoPP.getStringRepresentation(m.typeReference, m.arrayDimension))]
			for (candidate : candidates) {
				...
				for (var i = 0; i < candidate.parameters.size; i++) {
					...
					allParamsMatched = allParamsMatched && StringOperationsJaMoPP.getStringRepresentation(candParam.typeReference, candParam.arrayDimension)
						.equals(StringOperationsJaMoPP.getStringRepresentation(refParam.typeReference, refParam.arrayDimension))
				}
				if (allParamsMatched) {
