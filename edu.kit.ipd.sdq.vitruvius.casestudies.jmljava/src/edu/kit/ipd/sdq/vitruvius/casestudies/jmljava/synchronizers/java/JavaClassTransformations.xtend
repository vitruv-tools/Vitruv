...
class JavaClassTransformations extends Java2JMLTransformationBase {
	...
	override setCorrespondenceForFeatures() {
		featureCorrespondenceMap.put(ClassifiersPackage.eINSTANCE.class_.getEStructuralFeature("annotationsAndModifiers"), JMLPackage.eINSTANCE.modifiable_Modifiers)
		featureCorrespondenceMap.put(ClassifiersPackage.eINSTANCE.class_.getEStructuralFeature("members"), JMLPackage.eINSTANCE.normalClassDeclaration_BodyDeclarations)
	}
		...
		val jmlFeature = featureCorrespondenceMap.claimValueForKey(affectedReference)
		 ...
		 if (jmlFeature == JMLPackage.eINSTANCE.modifiable_Modifiers) {
			...
			if (jmlNormalClassDecl != null) {
				...
				if (jmlNormalClassDecl != null && MatchingModelElementsFinder.findMatchingMember(newValue as Commentable, jmlNormalClassDecl.getChildrenOfType(MemberDeclWithModifier)) != null) {
					LOGGER.info("Aborted transformation because of name clash with JML.")
					userInteracting.showMessage(UserInteractionType.MODAL, "There already is a " + newValue.class.simpleName.toLowerCase + " in JML, which has the same signature.");
				...
				if (newValue instanceof Method) {
					...
					LOGGER.warn("The given newValue has an unsupported type, so the change will be ignored.")
				}
	...
	override deleteNonRootEObjectInList(EObject newAffectedEObject, EObject oldAffectedEObject, EReference affectedReference, EObject oldValue, int index, EObject[] oldCorrespondingEObjectsToDelete) {
		...
		if (jmlFeature == JMLPackage.eINSTANCE.modifiable_Modifiers) {
			val jmlClassifierDecl = getSingleCorrespondingEObjectOfType(oldAffectedEObject, ClassifierDeclarationWithModifier)
			...
			if (jmlNormalClassDecl != null) {
				...
				if (!simulateElementRemoval(oldValue as Member)) {
					userInteracting.showMessage(UserInteractionType.MODAL, "The change is not allowed since this member is referenced in other methods or specifications.");
					return createTransformationChangeResultAborted
				}
...
/*
	override protected execute(UpdateEAttribute change, ModelInstance sourceModel, CorrespondenceInstance correspondenceInstance) {
