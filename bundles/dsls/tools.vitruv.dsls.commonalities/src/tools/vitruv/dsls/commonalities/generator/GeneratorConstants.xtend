...
@Utility class GeneratorConstants {
	...
	def static getIntermediateModelClass(CommonalityFile commonalityFile) {
		new GenericClassNameGenerator(commonalityFile.concept.name.conceptPackageFullName, commonalityFile.commonality.name)
	}
	...
	def static getConceptDomainProvider(String conceptName) {
		new GenericClassNameGenerator(conceptName.conceptPackageFullName, conceptName.conceptDomainName + "DomainProvider")
	}
}