			...
			ExpressionEvaluator ee = new ExpressionEvaluator(configuration) {
				...
				public <T extends Option> Boolean caseVariable(Variable<T> variable) {
					...
					return this.configuration.getOption().contains(variable.getOption())
							|| variable.getOption() instanceof Feature && this.configuration.getOption().stream().filter(o -> o instanceof FeatureRevision && ((FeatureRevision) o).eContainer().equals(variable.getOption())).findAny().isPresent()
