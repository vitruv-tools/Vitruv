...
public class CreateSonRoutine extends AbstractRepairRoutineRealization {
  private CreateSonRoutine.ActionUserExecution userExecution;
  ...
  private static class ActionUserExecution extends AbstractRepairRoutineRealization.UserExecution {
    ...
    public EObject getElement4(final Member member, final PersonRegister personsRegister, final Male person) {
      Family _familySon = member.getFamilySon();
      return _familySon;
    }
    ...
    public void updatePersonElement(final Member member, final PersonRegister personsRegister, final Male person) {
      ...
      String _lastName = member.getFamilySon().getLastName();
      String _plus_1 = (_plus + _lastName);
      person.setFullName(_plus_1);
    }
  }
  
  public CreateSonRoutine(final RoutinesFacade routinesFacade, final ReactionExecutionState reactionExecutionState, final CallHierarchyHaving calledBy, final Member member) {
    super(routinesFacade, reactionExecutionState, calledBy);
    this.userExecution = new mir.routines.familiesToPersons.CreateSonRoutine.ActionUserExecution(getExecutionState(), this);
    this.member = member;
  }
  ...
  protected boolean executeRoutine() throws IOException {
    getLogger().debug("Called routine CreateSonRoutine with input:");
