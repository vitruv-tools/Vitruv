...
class ChangePropagationResult {
	final Map<EObject, VURI> elementToPersistenceVuriMap
	...
	def Map<EObject, VURI> getElementToPersistenceMap() {
	...
	def void integrateResult(ChangePropagationResult transformationResult) {
		...
		for (EObject persistenceRootElement : transformationResult.elementToPersistenceVuriMap.keySet()) {
			this.registerForEstablishPersistence(persistenceRootElement,
				transformationResult.elementToPersistenceVuriMap.get(persistenceRootElement))
		}
	}
}
