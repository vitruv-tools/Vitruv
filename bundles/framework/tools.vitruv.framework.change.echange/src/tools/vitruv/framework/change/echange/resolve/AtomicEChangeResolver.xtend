...
class AtomicEChangeResolver {
	 ...
	 */
	def private static <A extends EObject, T extends EObject> EObject resolveNewValue(
		AdditiveReferenceEChange<A, T> change, ResourceSet resourceSet, boolean resolveBefore, boolean isList,
		int index) {
			...
			if (resolveBefore) {
				...
				return StagingArea.getStagingArea(change.affectedEObject.eResource).peek
			} else {
		 ...
		 */
		def private static <A extends EObject, T extends EObject> EObject resolveOldValue(
			SubtractiveReferenceEChange<A, T> change, ResourceSet resourceSet, boolean resolveBefore, boolean isList,
			int index) {
		...
		def package static dispatch boolean resolve(InsertRootEObject<EObject> change, ResourceSet resourceSet,
			...
			if (resolveBefore) {
				...
				change.newValue = StagingArea.getStagingArea(resourceSet).peek
			} else {
		...
		def package static dispatch boolean resolve(RemoveRootEObject<EObject> change, ResourceSet resourceSet,
			...
			// Resolve the root object
			if (resolveBefore) {
				// Root object is still in resource
				change.oldValue = EChangeUtil.resolveProxy(change.oldValue, resourceSet)
			} else {
				// Root object was removed and is in the staging area
				change.oldValue = StagingArea.getStagingArea(resourceSet).peek
			}
