...
public class ApplicationSelectionPage extends WizardPage {
  ...
  private static final String TARGETDOMAIN = "targetDomain";
  ...
  public void displayCheckedDomains(Map<IProject, HashSet<IExtension>> domains) {
    ...
    for (Collection<IExtension> collection : domains.values()) {
      for (IExtension ex : collection) {
        selectedDomainsIDs.add(ex.getUniqueIdentifier());
      }
    }
    ...
    for (IExtension ex : extensions) {
      for (IConfigurationElement config : ex.getConfigurationElements()) {
        String targetID = config.getAttribute(TARGETDOMAIN);
        String sourceID = config.getAttribute(SOURCEDOMAIN);
        if (null != tree
            && (selectedDomainsIDs.contains(targetID) || selectedDomainsIDs.contains(sourceID))) {
          toDraw.add(ex);
        }
      }
    }
