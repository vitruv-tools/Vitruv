...
abstract class AbstractRepairRoutineRealization extends CallHierarchyHaving implements RepairRoutine, ReactionElementsHandler {
	...
	public new(ReactionExecutionState executionState, CallHierarchyHaving calledBy) {
		...
		_reactionElementsHandler = new ReactionElementsHandlerImpl(correspondenceModel, transformationResult)
	}
	...
	protected def UserInteracting getUserInteracting() {
		executionState.userInteracting
	}

	protected def CorrespondenceModel getCorrespondenceModel() {
		executionState.correspondenceModel
	}
	...
	public static class UserExecution extends Loggable {
		...
		protected val ChangePropagationResult transformationResult
		protected val CorrespondenceModel correspondenceModel

		new(ReactionExecutionState executionState) {
			userInteracting = executionState.userInteracting
			transformationResult = executionState.transformationResult
			correspondenceModel = executionState.correspondenceModel
		}
