...
public class ChangeOperationRequiredRoleInterfaceEffect extends AbstractEffectRealization {
  ...
  protected void executeEffect() throws IOException {
    getLogger().debug("Called effect ChangeOperationRequiredRoleInterfaceEffect with input:");
    getLogger().debug("   UpdateSingleValuedNonContainmentEReference: " + this.change);
    
    preProcessElements();
    new mir.effects.pcm2java.ChangeOperationRequiredRoleInterfaceEffect.EffectUserExecution(getExecutionState(), this).executeUserOperations(
    	change);
    postProcessElements();
  }
