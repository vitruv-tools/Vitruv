...
public class RenamedInterfaceEffect extends AbstractEffectRealization {
  public RenamedInterfaceEffect(final ResponseExecutionState responseExecutionState, final CallHierarchyHaving calledBy) {
  ...
  protected void executeEffect() throws IOException {
    getLogger().debug("Called effect RenamedInterfaceEffect with input:");
    getLogger().debug("   UpdateSingleValuedEAttribute: " + this.change);
    
    preProcessElements();
    new mir.effects.pcm2java.RenamedInterfaceEffect.EffectUserExecution(getExecutionState(), this).executeUserOperations(
    	change);
    postProcessElements();
  }
