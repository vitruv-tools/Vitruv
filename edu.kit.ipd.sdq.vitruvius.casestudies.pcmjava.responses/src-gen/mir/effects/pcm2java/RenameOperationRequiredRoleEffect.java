...
public class RenameOperationRequiredRoleEffect extends AbstractEffectRealization {
  public RenameOperationRequiredRoleEffect(final ResponseExecutionState responseExecutionState, final CallHierarchyHaving calledBy) {
  ...
  protected void executeEffect() throws IOException {
    getLogger().debug("Called effect RenameOperationRequiredRoleEffect with input:");
    getLogger().debug("   UpdateSingleValuedEAttribute: " + this.change);
    
    preProcessElements();
    new mir.effects.pcm2java.RenameOperationRequiredRoleEffect.EffectUserExecution(getExecutionState(), this).executeUserOperations(
    	change);
    postProcessElements();
  }
