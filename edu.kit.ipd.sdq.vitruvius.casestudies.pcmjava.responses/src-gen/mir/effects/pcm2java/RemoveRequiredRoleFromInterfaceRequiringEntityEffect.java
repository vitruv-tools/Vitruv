...
public class RemoveRequiredRoleFromInterfaceRequiringEntityEffect extends AbstractEffectRealization {
  ...
  protected void executeEffect() throws IOException {
    getLogger().debug("Called effect RemoveRequiredRoleFromInterfaceRequiringEntityEffect with input:");
    getLogger().debug("   DeleteNonRootEObjectInList: " + this.change);
    
    preProcessElements();
    new mir.effects.pcm2java.RemoveRequiredRoleFromInterfaceRequiringEntityEffect.EffectUserExecution(getExecutionState(), this).executeUserOperations(
    	change);
    postProcessElements();
  }
