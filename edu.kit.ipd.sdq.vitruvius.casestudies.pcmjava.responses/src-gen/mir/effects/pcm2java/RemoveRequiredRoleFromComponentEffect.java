...
public class RemoveRequiredRoleFromComponentEffect extends AbstractEffectRealization {
  public RemoveRequiredRoleFromComponentEffect(final ResponseExecutionState responseExecutionState, final CallHierarchyHaving calledBy) {
  ...
  protected void executeEffect() throws IOException {
    getLogger().debug("Called effect RemoveRequiredRoleFromComponentEffect with input:");
    getLogger().debug("   DeleteNonRootEObjectInList: " + this.change);
    
    preProcessElements();
    new mir.effects.pcm2java.RemoveRequiredRoleFromComponentEffect.EffectUserExecution(getExecutionState(), this).executeUserOperations(
    	change);
    postProcessElements();
  }
