...
public class ChangedProvidingEntityOfProvidedRoleEffect extends AbstractEffectRealization {
  ...
  protected void executeEffect() throws IOException {
    getLogger().debug("Called effect ChangedProvidingEntityOfProvidedRoleEffect with input:");
    getLogger().debug("   UpdateSingleValuedNonContainmentEReference: " + this.change);
    
    preProcessElements();
    new mir.effects.pcm2java.ChangedProvidingEntityOfProvidedRoleEffect.EffectUserExecution(getExecutionState(), this).executeUserOperations(
    	change);
    postProcessElements();
  }
