...
public class RenamedCompositeDataTypeEffect extends AbstractEffectRealization {
  ...
  protected void executeEffect() throws IOException {
    getLogger().debug("Called effect RenamedCompositeDataTypeEffect with input:");
    getLogger().debug("   UpdateSingleValuedEAttribute: " + this.change);
    
    preProcessElements();
    new mir.effects.pcm2java.RenamedCompositeDataTypeEffect.EffectUserExecution(getExecutionState(), this).executeUserOperations(
    	change);
    postProcessElements();
  }
  ...
  private static class EffectUserExecution {
    ...
    private void executeUserOperations(final UpdateSingleValuedEAttribute<String> change) {
      ...
      this.effectFacade.callRenameCompositeDataType(((CompositeDataType) _newAffectedEObject));
    }
  }
}
