...
public class ChangeOperationRequiredRoleEntityEffect extends AbstractEffectRealization {
  ...
  protected void executeEffect() throws IOException {
    getLogger().debug("Called effect ChangeOperationRequiredRoleEntityEffect with input:");
    getLogger().debug("   UpdateSingleValuedNonContainmentEReference: " + this.change);
    
    preProcessElements();
    new mir.effects.pcm2java.ChangeOperationRequiredRoleEntityEffect.EffectUserExecution(getExecutionState(), this).executeUserOperations(
    	change);
    postProcessElements();
  }
