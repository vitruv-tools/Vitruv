...
public class CreateRootTestEffect extends AbstractEffectRealization {
  ...
  private String getModelPath(final CreateRootEObject<Root> change, final Root newRoot) {
    return "Further_Target_Test_Model";
  }
  ...
  protected void executeEffect() throws IOException {
    getLogger().debug("Called effect CreateRootTestEffect with input:");
    ...
    Root newRoot = initializeCreateElementState(
    	...
    	() -> null, // tag supplier
    	Root.class);
    setPersistenceInformation(newRoot, () -> getModelPath(change, newRoot), true);
    if (isAborted()) return;
    preProcessElements();
    new mir.effects.simpleChangesTests.CreateRootTestEffect.EffectUserExecution(getExecutionState(), this).executeUserOperations(
    	change, newRoot);
    postProcessElements();
  }
