...
/**
 * The default match engine does not take the hierarchical structure of the model into account. As the location of model elements is essential for our match strategies and moved elements are treated as separate ADD and DELETE, this match engine implements a "level by level" process.
 * <p>
 * <b>Two-Way Comparison only</b><br>
 * This match engine does not support three-way matches. So the origin part of the scop is ignored at all.
 * </p>
 */
public class HierarchicalMatchEngine implements IMatchEngine {
	 ...
	 *
	 * @param equalityHelper   The equality helper to check equality and to be wired with the comparison model.
	 * @param equalityStrategy The equality strategy to use.
	 * @param ignoreStrategy   the strategy which elements must not be matched and can be ignored.
	 * @param resourceMatcher  The matcher to decide if two resources belong to each other.
	 */
	public HierarchicalMatchEngine(IEqualityHelper equalityHelper, SimilarityChecker similarityChecker) {
