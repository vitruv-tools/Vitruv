...
class ChangeDescription2InsertEReferenceTest extends ChangeDescription2EReferenceTest {
	...
	def private testInsertInEReference(int expectedIndex) {
		...
		startRecording
		// test
		rootElement.multiValuedNonContainmentEReference.add(expectedIndex, nonRoot)
		// assert
	...
	def private void assertInsertEReference(NonRoot nonRoot, EStructuralFeature feature, int expectedIndex,
		...
		changes.claimChange(0).assertInsertEReference(rootElement, feature, nonRoot, expectedIndex, isContainment)
	}

	def private void assertCreateAndInsertNonRoot(NonRoot nonRoot, EStructuralFeature feature, int expectedIndex) {
		changes.assertChangeCount(2)
		changes.claimChange(0).assertCreateAndInsertNonRoot(rootElement, feature, nonRoot, expectedIndex)
		changes.claimChange(1).assertReplaceSingleValuedEAttribute(nonRoot, IDENTIFIED__ID, null, nonRoot.id)
	}
}
