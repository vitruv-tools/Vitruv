...
class ChangeDescriptionComplexSequencesTest extends ChangeDescription2ChangeTransformationTest {
	...
	def void testOverwritingSequence() {
		...
		changes.claimChange(0).assertSetSingleValuedEReference(rootElement, ROOT__SINGLE_VALUED_CONTAINMENT_EREFERENCE,
			nonRoot, true, true)
		changes.claimChange(1).assertReplaceSingleValuedEAttribute(nonRoot, IDENTIFIED__ID, null, nonRoot.id)
		changes.claimChange(2).assertUnsetSingleValuedEReference(rootElement,
			ROOT__SINGLE_VALUED_CONTAINMENT_EREFERENCE, nonRoot, true, true)
		changes.claimChange(3).assertSetSingleValuedEReference(rootElement, ROOT__SINGLE_VALUED_CONTAINMENT_EREFERENCE,
			nonRoot, true, true)
	...
	def void testInsertTreeInContainment() {
		// prepare
		rootElement.nonRootObjectContainerHelper = null
		startRecording
		...
		rootElement.nonRootObjectContainerHelper = nonRootObjectsContainer
		...
		changes.claimChange(1).assertReplaceSingleValuedEAttribute(nonRootObjectsContainer, IDENTIFIED__ID, null,
			nonRootObjectsContainer.id)
		val nonRootChange = changes.claimChange(2)
		nonRootChange.assertCreateAndInsertNonRoot(nonRootObjectsContainer,
			NON_ROOT_OBJECT_CONTAINER_HELPER__NON_ROOT_OBJECTS_CONTAINMENT, nonRoot, 0)
	...
	def public void testInsertComplexTreeInContainment() {
		...
		changes.claimChange(0).assertSetSingleValuedEReference(rootElement, ROOT__RECURSIVE_ROOT, secondRoot, true,
			true)
		changes.claimChange(1).assertReplaceSingleValuedEAttribute(secondRoot, IDENTIFIED__ID, null, secondRoot.id)
		changes.claimChange(2).assertCreateAndReplaceAndDeleteNonRoot(null, nonRootObjectsContainer,
			ROOT__NON_ROOT_OBJECT_CONTAINER_HELPER, secondRoot, true)
		changes.claimChange(3).assertReplaceSingleValuedEAttribute(nonRootObjectsContainer, IDENTIFIED__ID, null,
			nonRootObjectsContainer.id)
		changes.claimChange(4).assertCreateAndInsertNonRoot(nonRootObjectsContainer,
			NON_ROOT_OBJECT_CONTAINER_HELPER__NON_ROOT_OBJECTS_CONTAINMENT, nonRoot, 0)
		changes.claimChange(5).assertReplaceSingleValuedEAttribute(nonRoot, IDENTIFIED__ID, null, nonRoot.id)
		changes.claimChange(6).assertReplaceSingleValuedEReference(secondRoot,
			ROOT__SINGLE_VALUED_NON_CONTAINMENT_EREFERENCE, null, nonRoot, false)
	}
