...
class ChangeDescription2RemoveEReferenceTest extends ChangeDescription2EReferenceTest {
	...
	def private void testRemoveEReference(boolean isContainment, boolean isExplicitUnset) {
		...
		val feature = if (isExplicitUnset) {
				if (isContainment) {
		...
		(rootElement.eGet(feature) as List<NonRoot>).add(nonRoot)

		startRecording
		var EChange removeChange
		// test
		rootElement.eUnset(feature)
		...
		// assert 
		if (isContainment) {
			removeChange.assertRemoveAndDeleteNonRoot(rootElement, feature, nonRoot, 0)
		} else {
			removeChange.assertRemoveEReference(rootElement, feature, nonRoot, 0, isContainment)
		}
	}
	...
	def void testClearEReferences() {
		...
		rootElement.multiValuedContainmentEReference.clear

		changes.assertChangeCount(2)
		changes.claimChange(0).assertRemoveAndDeleteNonRoot(rootElement, ROOT__MULTI_VALUED_CONTAINMENT_EREFERENCE,
			index1Element, 1)
		changes.claimChange(1).assertRemoveAndDeleteNonRoot(rootElement, ROOT__MULTI_VALUED_CONTAINMENT_EREFERENCE,
			index0Element, 0)
	}
}
