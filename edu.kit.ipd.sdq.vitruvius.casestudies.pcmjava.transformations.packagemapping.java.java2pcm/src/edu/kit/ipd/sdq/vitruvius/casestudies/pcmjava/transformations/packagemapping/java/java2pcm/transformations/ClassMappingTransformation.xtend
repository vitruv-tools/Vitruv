...
class ClassMappingTransformation extends EmptyEObjectMappingTransformation {
	...
	override createEObject(EObject eObject) {
		...
		if (null != jaMoPPPackage) {
			...
			val pcmComponentOrSystems = correspondenceModel.getCorrespondingEObjectsByType(jaMoPPPackage,
	...
	def boolean alreadyHasClassCorrespondence(InterfaceProvidingRequiringEntity pcmComponentOrSystem) {
		var correspondencesForPCMCompOrSystem = correspondenceModel.
	...
	override removeEObject(EObject eObject) {
		...
		if (null != correspondences && 0 < correspondences.size) {
			...
			correspondenceModel.removeCorrespondencesThatInvolveAtLeastAndDependend(jaMoPPClass.toSet)
		}
		return null
	}
